...
chatRouter.post("/", JWTAuthMiddleware, async(req, res, next) => {
    try {
        ...
        const commonChat = await ChatSchema.find({$and:[{ members: { $in: [ requesterId ] }} , {members:{$in:[reciverId]}} ] })
            console.log(commonChat)
          
            if(commonChat.length > 0) {
                const updatedCommonChat = await ChatSchema.findByIdAndUpdate(commonChat[0]._id,
                    {$push:{history: message}},
                    {new:true})
                    console.log("upcomchat", updatedCommonChat)
...
chatRouter.get("/", JWTAuthMiddleware, async(req, res, next) => {
    try {
        const chatHistory = await ChatSchema.find({members:{ $in: [ req.user._id ] }})
